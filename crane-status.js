...
document.getElementById('crane-search-btn').addEventListener('click', async () => {
...
document.getElementById('crane-action').addEventListener('change', e => {
  document.getElementById('crane-reason-block').style.display =
    e.target.value === 'breakdown' ? 'block' : 'none';
});

document.getElementById('crane-submit-btn').addEventListener('click', async () => {
  const regNo = document.querySelector('#crane-status-form .reg_no').value.trim();
